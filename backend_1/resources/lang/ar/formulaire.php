<?php
return [
    'id_nin_personne'               => 'رقم الهوية الوطنية',
    'nom_personne_fr'               => 'الاسم (فرنسي)',
    'prenom_personne_fr'            => 'اللقب (فرنسي)',
    'nom_personne_ar'               => 'الاسم ',
    'prenom_personne_ar'            => 'اللقب ',
    'date_naissance'                => 'تاريخ الميلاد',
    'lieu_naissance_fr'             => 'مكان الولادة (فرنسي)',
    'lieu_naissance_ar'             => 'مكان الولادة ',
    'nationalite_fr'                => 'الجنسية (فرنسي)',
    'nationalite_ar'                => 'الجنسية ',
    'num_tlf_personne'              => 'الهاتف',
    'adresse_fr'                    => 'العنوان (فرنسي)',
    'adresse_ar'                    => 'العنوان ',
    'sexe_personne_fr'              => 'الجنس (فرنسي)',
    'sexe_personne_ar'              => 'الجنس ',
    'groupage'                      => 'فصيلة الدم',
    'carte_nationale'               => 'البطاقة الوطنية',
    'photo'                         => 'الصورة',
    'id_professional_card'          => 'رقم البطاقة المهنية',
    'num_attes'                     => 'رقم شهادة العمل',
    'fonction_fr'                   => 'الوظيفة (فرنسي)',
    'fonction_ar'                   => 'الوظيفة ',
    'secteur_travail'               => 'قطاع العمل',
    'categorie'                     => 'الفئة',
    'theme'                         => 'الموضوع',
    'collaborators'                 => 'المتعاونون',
    'type_media'                    => 'نوع الوسائط',
    'teamSize'                      => 'حجم الفريق',
    'role'                          => 'الدور',
    'file'                          => 'الملف(ات) العمل',
    'video_file'                    => 'العمل',
    'tv'                            => 'نوع التلفزيون',
    'radio'                         => 'نوع الراديو',
    'media'                         => 'نوع الوسائط المكتوبة',
    'langue'                        => 'لغة المؤسسة',
    'specialite'                    => 'التخصص',
    'nom_etablissement'             => 'اسم المؤسسة (فرنسي)',
    'nom_etablissement_ar'          => 'اسم المؤسسة ',
    'email'                         => 'البريد الإلكتروني للمؤسسة',
    'tel'                           => 'هاتف المؤسسة',
    'attestation_travail'           => 'شهادة العمل',
    'titre_oeuvre_fr'               => 'عنوان العمل (فرنسي)',
    'titre_oeuvre_ar'               => 'عنوان العمل ',
    'descriptif_oeuvre_fr'          => 'وصف العمل (فرنسي)',
    'descriptif_oeuvre_ar'          => 'وصف العمل ',
    'date_publication'              => 'تاريخ النشر',
    'video_url'                     => 'رابط فيديو يوتيوب',
    'required'                      => 'الحقل :attribute مطلوب.',
    'nin_invalid'                   => 'رقم الهوية الوطنية يجب أن يحتوي على 18 رقمًا.',
    'phone_invalid'                 => 'رقم الهاتف يجب أن يحتوي على ما بين 8 و15 أرقام.',
    'email_invalid'                 => 'البريد الإلكتروني غير صالح أو مطلوب.',
    'professional_card_required'    => 'رقم البطاقة المهنية مطلوب.',
    'professional_card_exists'      => 'هذه البطاقة المهنية تخص شخصًا آخر بالفعل.',
    'professional_card_found'       => 'تم العثور على البطاقة المهنية. يمكنك تعديل البيانات.',
    'new_professional_card'         => 'بطاقة مهنية جديدة. يرجى ملء المعلومات.',
    'error_check_professional_card' => 'خطأ أثناء التحقق من البطاقة المهنية.',
    'invalid_category'              => 'الفئة مطلوبة ويجب أن تكون "وسائط صوتية" أو "وسائط مكتوبة وإلكترونية".',
    'invalid_media_type'            => 'نوع الوسائط مطلوب ويجب أن يكون "تلفزيون" أو "راديو".',
    'invalid_tv_type'               => 'نوع التلفزيون مطلوب ويجب أن يكون "إقليمي" أو "وطني".',
    'invalid_radio_type'            => 'نوع الراديو مطلوب ويجب أن يكون "عمومي" أو "محلي".',
    'invalid_written_media_type'    => 'نوع الوسائط المكتوبة مطلوب ويجب أن يكون "مكتوب" أو "إلكتروني".',
    'invalid_category_private'      => 'يجب أن تكون الفئة "خاص" للقطاع الخاص.',
    'invalid_media_type_private'    => 'يجب أن يكون نوع الوسائط "خاص" للقطاع الخاص.',
    'max_video_size'                => 'حجم الفيديو لا يجب أن يتجاوز 100 ميغابايت.',
    'video_duration_exceeded'       => 'مدة الفيديو لا يجب أن تتجاوز 13 دقيقة.',
    'invalid_youtube_url'           => 'الرابط يجب أن يكون رابط يوتيوب صالح.',
    'video_or_url_required'         => 'يجب تقديم إما فيديو أو رابط يوتيوب، لكن ليس كليهما.',
    'next_step'                     => 'الخطوة التالية →',
    'prev_step'                     => '← الخطوة السابقة',
    'logout'                        => 'تسجيل الخروج',
    'select_language'               => 'اختر اللغة',
    'search'                        => ':بحث',
    'search_placeholder'            => 'بحث...',
    'already_submitted'             => 'لقد قمت بالفعل بتقديم :attribute مع هذا الفريق.',
    'forme_not_found'               => 'لم يتم العثور على نموذج نشط لهذا الشخص.',
    'max_files'                     => 'لا يمكن أن يتجاوز الحقل :attribute :max ملفات.',
    'max_length'                    => 'لا يمكن أن يتجاوز الحقل :attribute :max أحرف.',
    'step3_saved'                   => 'تم حفظ المرحلة 3 بنجاح.',
    'save_error'                    => 'خطأ أثناء الحفظ: :message',
    "num_attes_exists"              => "رقم شهادة العمل موجود مسبقًا.",
    "error_check_num_attes"         => "خطأ أثناء التحقق من رقم شهادة العمل.",
    'nin_belongs_to_another_user'   => 'رقم الهوية الوطنية هذا يخص مستخدمًا آخر.',
    'carte_professionnelle'         => 'البطاقة المهنية',
    'file_prohibited_if_url'        => 'لا يمكنك رفع ملف إذا تم تقديم رابط فيديو.',
    'url_prohibited_if_file'        => 'لا يمكنك تقديم رابط فيديو إذا تم رفع ملف.',
    'file_or_url_required'          => 'يجب تقديم إما ملف أو رابط فيديو.',
    'url_required_if_no_file'       => 'رابط الفيديو مطلوب إذا لم يتم تقديم ملف.',
    'file_required_if_no_url'       => 'الملف مطلوب إذا لم يتم تقديم رابط فيديو.',
    'invalid_file_type'             => 'نوع الملف غير مسموح لـ :attribute.',
    'max_file_size'                 => 'يجب ألا يتجاوز الملف :attribute الحجم Mo :max.',
    'certificate'                   => 'شهادة النشر',
    'file_name_too_long'            => 'اسم :attribute طويل جدًا (حد أقصى :max حرفًا).',
    'max_total_file_size'           => 'يجب ألا يتجاوز الحجم الإجمالي للملفات :max ميغابايت.',
    'attestation_number_required'   => 'Le numéro d\'attestation est requis.',
    'attestation_number_exists'     => 'Ce numéro d\'attestation est déjà utilisé par un autre utilisateur.',
    'attestation_number_found'      => 'Numéro d\'attestation trouvé. Les données associées ont été chargées.',
    'attestation_number_not_found'  => 'Nouveau numéro d\'attestation détecté.',
];
