...
// Charger les données du formulaire (seulement si pas déjà chargées)
useEffect(() => {
  const fetchFormData = async () => {
    // Garde : ne re-fetch pas si déjà des données (évite d'overwrite sur changement de langue)
  ...
  fetchFormData();
}, [translations, formData.userId]);  // Gardez [translations] pour re-run sur langue, mais la garde empêche l'overwrite
  ...
  const handleFileChange = (e) => {
    ...
    if (files.length > 0) {
      ...
      if (name === "carte_nationale" && files[0].size > maxSizeCarteNationale) {
        setError(
          translations.max_file_size?.replace(
            ":attribute",
      ...
      if (
        ...
        files[0].size > maxSizeAttestation
      ) {
        setError(
          translations.max_file_size?.replace(
            ":attribute",
            translations.attestation_travail || "attestation de travail"
          ) ||
            (interfaceLocale === "fr"
              ...
              >
                <span
                  className={`flex items-center after:content-['/'] sm:after:hidden ${
                    direction === "rtl" ? "after:mr-3" : "after:ml-3"
              ...
              <li
                className={`flex md:w-full items-center ${
                  step === 2 || step > 2
                    ? "text-blue-600 dark:text-blue-500"
                    : ""
                } sm:after:content-[''] after:w-full after:h-1 after:border-b after:border-gray-200 after:border-1 after:hidden sm:after:inline-block ${
                  direction === "rtl"
