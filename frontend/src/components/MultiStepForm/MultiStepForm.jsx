...
export default function MultiStepForm({
  ...
  const [isProfessionalCardValidated, setIsProfessionalCardValidated] =
    useState(false);
    ...
    const fetchTranslations = async () => {
      try {
        const response = await fetch(
          `http://localhost:8000/translations/${interfaceLocale}`,
          {
            credentials: "include",
    ...
    if (e.target.name === "batch") {
      ...
      setFormData((prev) => ({
        ...
        ...(name === "secteur_travail" && value === "Privé"
          ? { categorie: "Privé", type_media: "Privé" }
          : {}),
        ...(name === "secteur_travail" && value === "Public"
          ? { categorie: "", type_media: "" }
          : {}),
      }));
    }
    setError("");
  ...
  const handleFileChange = (e) => {
    ...
    if (files.length > 0) {
      ...
      const maxSizeCarteNationale = 2 * 1024 * 1024; // 2 Mo pour carte_nationale

      const maxSizePhoto = 2 * 1024 * 1024; // 2 Mo pour photo

      const maxSizeAttestation = 10 * 1024 * 1024; // 10 Mo pour attestation_travail

      const maxSizeFile = 100 * 1024 * 1024; // 100 Mo pour autres fichiers



      if (name === 'carte_nationale' && files[0].size > maxSizeCarteNationale) {

        setError(

          translations.max_file_size?.replace(':attribute', translations.carte_nationale || 'carte nationale') ||
      ...
      if (name === 'photo' && files[0].size > maxSizePhoto) {

        setError(

          translations.max_file_size?.replace(':attribute', translations.photo || 'photo') ||
      ...
      if (name === 'attestation_travail' && files[0].size > maxSizeAttestation) {

        setError(

          translations.max_file_size?.replace(':attribute', translations.attestation_travail || 'attestation de travail') ||
        ...
        for (let i = 0; i < files.length; i++) {
          ...
          if (files[i].size > maxSizeFile) {
            ...
            setError(
              ...
              translations.max_video_size?.replace(':attribute', `fichier ${files[i].name}`) ||

              (interfaceLocale === 'fr'

                ? `La taille du fichier ${files[i].name} ne doit pas dépasser 100 Mo.`

                : `حجم الملف ${files[i].name} يجب ألا يتجاوز 100 ميغابايت.`)
  ...
  const validateStep1 = () => {
    ...
    if (
      step1Data.id_nin_personne.length !== 18 ||
      !/^[0-9]{18}$/.test(step1Data.id_nin_personne)
    ) {
      setError(
        translations.nin_invalid || "Le numéro NIN doit contenir 18 chiffres."
      );
      return false;
    }
      ...
      <div className="flex-grow flex flex-col items-center justify-start p-4">
        <div className="bg-white rounded-xl shadow-xl w-full max-w-5xl overflow-hidden">
          <div className="p-6 border-b border-gray-200">
            <ol
              className={`flex items-center w-full text-sm font-medium text-center text-gray-500 dark:text-gray-400 sm:text-base ${direction === "rtl"
                ? "flex-row-reverse space-x-reverse space-x-6"
                : "flex-row space-x-6"
                }`}
              style={{
                direction: direction === "rtl" ? "rtl !important" : "ltr",
              }}
            >
              <li
              ...
              >
                <span
                  className={`flex items-center after:content-['/'] sm:after:hidden ${direction === "rtl" ? "after:mr-3" : "after:ml-3"
                    } after:text-gray-200 dark:after:text-gray-500`}
                >
                  {interfaceLocale === "ar" ? (
                    <>
                      {translations.step1_title ||
                        (interfaceLocale === "fr"
              ...
              <li
                className={`flex md:w-full items-center ${step === 2 || step > 2
                  ? "text-blue-600 dark:text-blue-500"
                  : ""
                  } sm:after:content-[''] after:w-full after:h-1 after:border-b after:border-gray-200 after:border-1 after:hidden sm:after:inline-block ${direction === "rtl"
                    ? "after:mr-8 xl:after:mr-12"
                    : "after:ml-8 xl:after:ml-12"
              ...
              >
                <span
                  className={`flex items-center ${direction === "rtl" ? "after:mr-3" : "after:ml-3"
                    } after:content-['/'] sm:after:hidden after:text-gray-200 dark:after:text-gray-500`}
                >
                  {interfaceLocale === "ar" ? (
                    <>
                      {translations.step3_title ||
                        (interfaceLocale === "fr"
